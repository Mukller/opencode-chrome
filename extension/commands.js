...
async function handleCommand(cmd, p) {
  switch (cmd) {
    ...
    case "get_computed_style": { const id = await requireTab(p); return await cdpGetComputedStyle(id, String(p.selector)); }
    ...
    case "cookie_set": { const id = await requireTab(p); return await cdpSetCookie(id, String(p.name), String(p.value), p.domain); }
    case "local_storage": { const id = await requireTab(p); return await cdpLocalStorage(id, String(p.action||"get"), p.key, p.value); }
    case "export_session": { const id = await requireTab(p); return await cdpExportSession(id); }
    ...
    case "batch": { const id = await requireTab(p); const ops = typeof p.operations === "string" ? JSON.parse(p.operations) : p.operations; return await cdpBatch(id, ops); }
    case "if_exists": { const id = await requireTab(p); const thenA = typeof p.thenAction === "string" ? JSON.parse(p.thenAction) : p.thenAction; const elseA = typeof p.elseAction === "string" ? JSON.parse(p.elseAction) : p.elseAction; return await cdpIfExists(id, String(p.selector), thenA, elseA); }
